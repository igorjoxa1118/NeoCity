...
complete -c gvim -o dev -d 'Open {device} for use as a terminal.  Only on the Amiga'
...
complete -c gvim -s P -d 'Win32 GUI only: Specify the title of the parent application'
...
complete -c gvim -s x -d 'If  Vim has been compiled with encryption support, use encryption when writin…'
...
complete -c gvim -l gui-dialog-file -d 'When using the GUI, instead of showing a dialog, write the title and message …'
complete -c gvim -l help -s h -s '?' -d 'Give a bit of help about the command line arguments and options'
complete -c gvim -l literal -d 'Take file name arguments literally, do not expand wildcards'
complete -c gvim -l log -d 'If  Vim has been compiled with eval and channel feature, start logging and wr…'
complete -c gvim -l nofork -d Foreground
complete -c gvim -l noplugin -d 'Skip loading plugins.   Implied by -u NONE'
complete -c gvim -l not-a-term -d 'Tells  Vim that the user knows that the input and/or output is not connected …'
...
complete -c gvim -l ttyfail -d 'When stdin or stdout is not a a terminal (tty) then exit right away'
...
complete -c gvim -l windowid -d 'Win32 GUI only: Make gVim try to use the window {id} as a parent, so that it …'
