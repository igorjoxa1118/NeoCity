...
complete -c mpv -l alang -d 'Specify a prioritized list of audio languages to use, as IETF language tags'
complete -c mpv -l slang -d 'Equivalent to --alang, for subtitle tracks. sp This is a string list option'
...
complete -c mpv -l subs-with-matching-audio -d 'When autoselecting a subtitle track, select it even if the selected audio str…'
complete -c mpv -l subs-match-os-language -d 'When autoselecting a subtitle track, select the track that matches the langua…'
complete -c mpv -l subs-fallback -d 'When autoselecting a subtitle track, if no tracks match your preferred langua…'
complete -c mpv -l subs-fallback-forced -d 'When autoselecting a subtitle track, the default value of yes will prefer usi…'
...
complete -c mpv -l pitch -d 'Raise or lower the audio\'s pitch by the factor given as parameter'
...
complete -c mpv -l ordered-chapters -d 'Enable support for Matroska ordered chapters'
...
complete -c mpv -l play-direction -s '>' -d 'Control the playback direction (default: forward)'
...
complete -c mpv -l no-config -d 'Do not load default configuration or any user files'
...
complete -c mpv -l load-scripts -d 'If set to no, don\'t auto-load scripts from the scripts configuration subdirec…'
...
complete -c mpv -l ytdl -d 'Enable the youtube-dl hook-script'
...
complete -c mpv -l js-memory-report -d 'Enable memory reporting for javascript scripts in the stats overlay'
complete -c mpv -l load-stats-overlay -d 'Enable the builtin script that shows useful playback information on a key bin…'
complete -c mpv -l load-osd-console -d 'Enable the built-in script that shows a console on a key binding and lets you…'
complete -c mpv -l load-auto-profiles -d 'Enable the builtin script that does auto profiles (default: auto)'
complete -c mpv -l load-select -d 'Enable the builtin script that lets you select from lists of items (default: …'
...
complete -c mpv -l watch-later-dir -d 'The directory in which to store the "watch later" temporary files'
complete -c mpv -l resume-playback -d 'Restore playback position from the watch_later configuration subdirectory, us…'
...
complete -c mpv -l display-fps-override -d 'Set the display FPS used with the --video-sync=display-* modes'
...
complete -c mpv -l video-crop -l video-crop -d 'Crop the video by starting at the x, y offset for w, h pixels'
...
complete -c mpv -l correct-pts -d '--correct-pts=no switches mpv to a mode where video timing is determined usin…'
complete -c mpv -l container-fps-override -d 'Override video framerate.  Useful if the original value is wrong or missing'
complete -c mpv -l deinterlace -d 'Enable or disable deinterlacing (default: no)'
complete -c mpv -l deinterlace-field-parity -d 'Specify the field parity/order when deinterlacing (default: auto)'
...
complete -c mpv -l vd-lavc-film-grain -d 'Enables film grain application on the GPU'
complete -c mpv -l vd-lavc-dr -d 'Enable direct rendering (default: auto)'
...
complete -c mpv -l vd-lavc-skiploopfilter -d 'Skips the loop filter (AKA deblocking) during decoding'
...
complete -c mpv -l vd-apply-cropping -d 'Certain video codecs support cropping, meaning that only a sub-rectangle of t…'
...
complete -c mpv -l volume-max -d 'Set the maximum amplification level in percent (default: 130)'
complete -c mpv -l volume-gain -d 'Set the volume gain in dB'
complete -c mpv -l volume-gain-max -l volume-gain-min -d 'Set the volume gain range in dB (default: -96 dB min, 12 dB max)'
...
complete -c mpv -l replaygain-clip -d 'Allow the volume gain to clip (default: no)'
...
complete -c mpv -l mute -d 'Set startup audio mute status (default: no). sp See also: --volume'
...
complete -c mpv -l ad-spdif-dtshd -l dtshd -d 'If DTS is passed through, use DTS-HD. sp WARNING: NDENT 7. 0 NDENT 3'
...
complete -c mpv -l initial-audio-sync -d 'When starting a video file or after events such as seeking, mpv will by defau…'
complete -c mpv -l audio-file-auto -d 'Load additional audio files matching the video filename'
complete -c mpv -l audio-exts -d 'Audio file extentions to try to match when using --audio-file-auto, --autocre…'
...
complete -c mpv -l sub-lavc-o -d 'Pass AVOptions to libavcodec decoder'
complete -c mpv -l sub-delay -d 'Delays primary subtitles by <sec> seconds.  Can be negative'
complete -c mpv -l secondary-sub-delay -d 'Delays secondary subtitles by <sec> seconds.  Can be negative'
...
complete -c mpv -l sub-scale-with-window -d 'Make the subtitle font size relative to the window (default: yes)'
...
complete -c mpv -l secondary-sub-pos -d 'Specify the position of secondary subtitles on the screen'
complete -c mpv -l sub-speed -d 'Multiply the subtitle event timestamps with the given value'
complete -c mpv -l sub-ass-style-overrides -d 'Override some style or script info parameters'
...
complete -c mpv -l secondary-sub-ass-override -d 'Control whether user secondary substyle overrides should be applied'
...
complete -c mpv -l sub-ass-use-video-data -d 'Controls which information about the video stream is passed to libass'
complete -c mpv -l sub-ass-video-aspect-override -d 'Allows passing any arbitrary aspect ratio to libass instead of the videoâs …'
complete -c mpv -l sub-vsfilter-bidi-compat -d 'Set implicit bidi detection to ltr instead of auto to match ASS\' default'
...
complete -c mpv -l sub-ass -d 'Render ASS subtitles natively (default: yes). sp NOTE: NDENT 7. 0 NDENT 3'
complete -c mpv -l sub-auto -d 'Load additional subtitle files matching the video filename'
complete -c mpv -l sub-auto-exts -d 'Subtitle extentions to try and match when using --sub-auto'
complete -c mpv -l sub-codepage -d 'You can use this option to specify the subtitle codepage'
complete -c mpv -l sub-stretch-durations -d 'Stretch a subtitle duration so it ends when the next one starts'
complete -c mpv -l sub-fix-timing -d 'Adjust subtitle timing is to remove minor gaps or overlaps between subtitles …'
complete -c mpv -l sub-forced-events-only -d 'Enabling this displays only forced events within subtitle streams'
...
complete -c mpv -l sub-visibility -d 'Can be used to disable display of subtitles, but still select and decode them'
complete -c mpv -l secondary-sub-visibility -d 'Can be used to disable display of secondary subtitles, but still select and d…'
complete -c mpv -l sub-clear-on-seek -d '(Obscure, rarely useful'
complete -c mpv -l teletext-page -d 'Select a teletext page number to decode'
...
complete -c mpv -l sub-blur -d 'Gaussian blur factor applied to the sub font border'
complete -c mpv -l sub-bold -d 'Format text on bold'
complete -c mpv -l sub-italic -d 'Format text on italic'
complete -c mpv -l sub-outline-color -d 'See --sub-color.  Color used for the sub font outline'
complete -c mpv -l sub-back-color -d 'See --sub-color.  Color used for sub text background'
complete -c mpv -l sub-outline-size -d 'Size of the sub font outline in scaled pixels (see --sub-font-size for detail…'
complete -c mpv -l sub-border-style -d 'The style of the border.  NDENT 7. 0'
...
complete -c mpv -l sub-filter-sdh-enclosures -d 'Specify a string of characters that --sub-filter-sdh will use to potentially …'
...
complete -c mpv -l sub-fonts-dir -d 'Font files in this directory are used by mpv/libass for subtitles'
...
complete -c mpv -l taskbar-progress -d '(Windows only) Enable/disable playback progress rendering in taskbar (Windows…'
complete -c mpv -l snap-window -d '(Windows only) Snap the player window to screen edges'
complete -c mpv -l drag-and-drop -d 'Controls the default behavior of drag and drop on platforms that support this'
...
complete -c mpv -l focus-on -d '(macOS only) Focus the video window and make it the front most window on spec…'
complete -c mpv -l window-corners -d '(Windows only) Set the preference for window corner rounding.  NDENT 7. 0'
complete -c mpv -l border -d 'Play video with window border and decorations'
complete -c mpv -l title-bar -d '(Windows and X11 only) Play video with the window title bar'
...
complete -c mpv -l autofit-larger -d 'This option behaves exactly like --autofit, except that it sets the maximum s…'
...
complete -c mpv -l cursor-autohide -d 'Make mouse cursor automatically hide after given number of milliseconds (defa…'
...
complete -c mpv -l force-render -d 'Forces mpv to always render frames regardless of the visibility of the window'
complete -c mpv -l force-window-position -d 'Forcefully move mpv\'s video output window to default location whenever there …'
complete -c mpv -l auto-window-resize -d 'By default, mpv will automatically resize itself if the video\'s size changes …'
complete -c mpv -l keepaspect -d '--keepaspect=no will always stretch the video to window size, and will disabl…'
complete -c mpv -l keepaspect-window -d '--keepaspect-window=yes (the default) will lock the window size to the video …'
complete -c mpv -l monitoraspect -d 'Set the aspect ratio of your monitor or TV screen'
complete -c mpv -l hidpi-window-scale -d 'Scale the window size according to the backing DPI scale factor from the OS (…'
complete -c mpv -l native-fs -d '(macOS only) Uses the native fullscreen mechanism of the OS (default: yes)'
complete -c mpv -l show-in-taskbar -d '(Windows and X11 only) Show mpv in the taskbar (default: yes)'
complete -c mpv -l monitorpixelaspect -d 'Set the aspect of a single pixel of your monitor or TV screen (default: 1)'
complete -c mpv -l stop-screensaver -d 'Turns off the screensaver (or screen blanker and similar mechanisms) at start…'
complete -c mpv -l wid -d 'This tells mpv to attach to an existing window'
complete -c mpv -l window-dragging -d 'Move the window when clicking on it and moving the mouse pointer (default: ye…'
complete -c mpv -l x11-name -d 'Set the window instance name for X11-based video output methods'
...
complete -c mpv -l x11-present -d 'Whether or not to use presentation statistics from X11\'s presentation extensi…'
complete -c mpv -l x11-wid-title -d 'Whether or not to set the window title when mpv is embedded on X11 (default: …'
complete -c mpv -l cdda-device -d 'Specify the CD device for CDDA playback (default: /dev/cdrom)'
...
complete -c mpv -l demuxer-mkv-subtitle-preroll -d 'Try harder to show embedded soft subtitles when seeking somewhere'
...
complete -c mpv -l demuxer-hysteresis-secs -d 'Once the demuxer limit is reached (--demuxer-max-bytes, --demuxer-readahead-s…'
...
complete -c mpv -l directory-mode -d 'When opening a directory, open subdirectories lazily, recursively or not at a…'
complete -c mpv -l directory-filter-types -d 'Media file types to filter when opening directory'
complete -c mpv -l autocreate-playlist -d 'When opening a local file, act as if the parent directory is opened and creat…'
complete -c mpv -l native-keyrepeat -d 'Use system settings for keyrepeat delay and rate, instead of --input-ar-delay…'
complete -c mpv -l native-touch -d '(Windows only) For platforms which send emulated mouse inputs for touch-unawa…'
complete -c mpv -l input-ar-delay -d 'Delay in milliseconds before we start to autorepeat a key (default: 200)'
complete -c mpv -l input-ar-rate -d 'Number of key presses to generate per second on autorepeat (default: 40)'
complete -c mpv -l input-conf -d 'Specify input configuration file other than the default location in the mpv c…'
complete -c mpv -l input-default-bindings -d 'Enable default-level ("weak") key bindings (default: yes)'
complete -c mpv -l input-builtin-bindings -d 'Enable loading of built-in key bindings during start-up (default: yes)'
complete -c mpv -l input-builtin-dragging -d 'Enable the built-in window-dragging behavior (default: yes)'
complete -c mpv -l input-cmdlist -d 'Prints all commands that can be bound to keys'
complete -c mpv -l input-commands -d 'Define a list of commands for mpv to run'
...
complete -c mpv -l input-terminal -d '--input-terminal=no prevents the player from reading key events from standard…'
...
complete -c mpv -l input-cursor -d 'Permit mpv to receive pointer events reported by the video output driver'
complete -c mpv -l input-cursor-passthrough -d 'Tell the backend windowing system to allow pointer events to passthrough the …'
complete -c mpv -l input-media-keys -d 'On systems where mpv can choose between receiving media keys or letting the s…'
complete -c mpv -l input-preprocess-wheel -d 'Preprocess WHEEL_* events so that while scrolling on the horizontal or vertic…'
complete -c mpv -l input-right-alt-gr -d '(macOS and Windows only) Use the right Alt key as Alt Gr to produce special c…'
complete -c mpv -l input-vo-keyboard -d 'Disable all keyboard input on for VOs which can\'t participate in proper keybo…'
complete -c mpv -l input-touch-emulate-mouse -d 'When multi-touch support is enabled (either required by the platform, or enab…'
complete -c mpv -l input-dragging-deadzone -d 'Begin the built-in window dragging when the mouse moves outside a deadzone of…'
complete -c mpv -l osc -d 'Whether to load the on-screen-controller (default: yes)'
complete -c mpv -l osd-bar -d 'Enable display of the OSD bar (default: yes)'
...
complete -c mpv -l osd-playing-msg-duration -d 'Set the duration of osd-playing-msg in ms'
complete -c mpv -l osd-playlist-entry -d 'Whether to display the media title, filename, or both'
...
complete -c mpv -l osd-bar-outline-size -d 'Size of the outline of the OSD bar in scaled pixels (see --sub-font-size for …'
complete -c mpv -l osd-blur -d 'Gaussian blur factor applied to the OSD font border'
complete -c mpv -l osd-bold -d 'Format text on bold'
complete -c mpv -l osd-italic -d 'Format text on italic'
complete -c mpv -l osd-outline-color -d 'See --sub-color.  Color used for the OSD font outline'
complete -c mpv -l osd-back-color -d 'See --sub-color.  Color used for OSD text background'
complete -c mpv -l osd-outline-size -d 'Size of the OSD font outline in scaled pixels (see --sub-font-size for detail…'
complete -c mpv -l osd-border-style -d 'See --sub-border-style.  Style used for OSD text border'
...
complete -c mpv -l osd-fonts-dir -d 'See --sub-fonts-dir for details.   Defaults to ~~/fonts'
complete -c mpv -l screenshot-format -d 'Set the image file type used for saving screenshots'
complete -c mpv -l screenshot-tag-colorspace -d 'Tag screenshots with the appropriate colorspace (default: yes)'
...
complete -c mpv -l screenshot-dir -d 'Store screenshots in this directory'
...
complete -c mpv -l screenshot-jxl-distance -d 'Set the JPEG XL Butteraugli distance.  Lower means better quality'
complete -c mpv -l screenshot-jxl-effort -d 'Set the JPEG XL compression effort'
complete -c mpv -l screenshot-avif-encoder -d 'Specify the AV1 encoder to be used by libavcodec for encoding avif screenshots'
complete -c mpv -l screenshot-avif-pixfmt -d 'Specify the pixel format for the libavcodec encoder'
complete -c mpv -l screenshot-avif-opts -d 'Specifies libavcodec options for selected encoder'
...
complete -c mpv -l terminal -d '--terminal=no disables any use of the terminal and stdin/stdout/stderr'
complete -c mpv -l msg-color -d 'Enable colorful console output on terminals (default: yes)'
...
complete -c mpv -l term-osd-bar -d 'Enable printing a progress bar under the status line on the terminal'
...
complete -c mpv -l demuxer-cache-dir -d 'Directory where to create temporary files'
...
complete -c mpv -l demuxer-cache-unlink-files -d 'Whether or when to unlink cache files (default: immediate)'
...
complete -c mpv -l cookies -d 'Support cookies when making HTTP requests.  Disabled by default'
...
complete -c mpv -l scale-blur -l cscale-blur -l dscale-blur -l tscale-blur -d 'Kernel scaling factor (also known as a blur factor)'
...
complete -c mpv -l interpolation-preserve -d 'Preserve the previous frames\' interpolated results even when renderer paramet…'
complete -c mpv -l opengl-pbo -d 'Enable use of PBOs'
complete -c mpv -l dither-depth -d 'Set dither target depth to N.  Default: auto.  NDENT 7. 0'
...
complete -c mpv -l egl-config-id -d '(EGL only) Select EGLConfig with specific EGL_CONFIG_ID'
complete -c mpv -l egl-output-format -d '(EGL only) Select a specific EGL output format to utilize for OpenGL rendering'
complete -c mpv -l vulkan-device -d 'The name or UUID of the Vulkan device to use for rendering and presentation'
...
complete -c mpv -l d3d11-exclusive-fs -d 'Switches the D3D11 swap chain fullscreen state to \'fullscreen\' when fullscree…'
...
complete -c mpv -l wayland-configure-bounds -d 'Controls whether or not mpv opts into the configure bounds event if sent by t…'
complete -c mpv -l wayland-content-type -d 'If supported by the compositor, mpv will send a hint using the content-type p…'
complete -c mpv -l wayland-disable-vsync -d 'Disable mpv\'s internal vsync for Wayland-based video output (default: no)'
complete -c mpv -l wayland-edge-pixels-pointer -d 'Defines the size of an edge border (default: 16) to initiate client side resi…'
complete -c mpv -l wayland-edge-pixels-touch -d 'Defines the size of an edge border (default: 32) to initiate client side resi…'
complete -c mpv -l wayland-present -d 'Enable the use of wayland\'s presentation time protocol for more accurate fram…'
...
complete -c mpv -l glsl-shader-opts -d 'Specifies the options to use for tunable shader parameters'
...
complete -c mpv -l deband-threshold -d 'The debanding filter\'s cut-off threshold'
complete -c mpv -l deband-range -d 'The debanding filter\'s initial radius'
complete -c mpv -l deband-grain -d 'Add some extra noise to the image'
complete -c mpv -l corner-rounding -d 'If set to a value above 0'
...
complete -c mpv -l opengl-dwmflush -d '(Windows only) Calls DwmFlush after swapping buffers on Windows (default: aut…'
...
complete -c mpv -l angle-egl-windowing -d 'Use ANGLE\'s built in EGL windowing functions to create a swap chain (default:…'
...
complete -c mpv -l cocoa-cb-output-csp -d 'This sets the color space of the layer to activate the macOS color transforma…'
...
complete -c mpv -l macos-render-timer -d 'Sets the mode (default: callback) for syncing the rendering of frames to the …'
complete -c mpv -l macos-menu-shortcuts -d 'Enables the default menu bar shortcuts (default: yes)'
...
complete -c mpv -l gpu-context -d 'Specify a priority list of the GPU contexts to be used'
complete -c mpv -l gpu-api -d 'Specify a priority list of accepted graphics APIs.  NDENT 7. 0'
...
complete -c mpv -l image-lut -d 'Specifies a custom LUT file (in Adobe '
complete -c mpv -l image-lut-type -d 'Controls the interpretation of color values fed to and from the LUT specified…'
complete -c mpv -l target-colorspace-hint -d 'Automatically configure the output colorspace of the display to pass through …'
...
complete -c mpv -l target-contrast -d 'Specifies the measured contrast of the output display'
complete -c mpv -l target-gamut -d 'Constrains the gamut of the display.  You can use this option to output e. g'
complete -c mpv -l target-lut -d 'Specifies a custom LUT file (in Adobe '
...
complete -c mpv -l inverse-tone-mapping -d 'If set, allows inverse tone mapping (expanding SDR to HDR)'
complete -c mpv -l tone-mapping-max-boost -d 'Upper limit for how much the tone mapping algorithm is allowed to boost the a…'
complete -c mpv -l tone-mapping-visualize -d 'Display a (PQ-PQ) graph of the active tone-mapping LUT'
complete -c mpv -l gamut-mapping-mode -d 'Specifies the algorithm used for reducing the gamut of images for the target …'
complete -c mpv -l hdr-compute-peak -d 'Compute the HDR peak and frame average brightness per-frame instead of relyin…'
complete -c mpv -l allow-delayed-peak-detect -d 'When using --hdr-compute-peak, allow delaying the detected peak by a frame wh…'
complete -c mpv -l hdr-peak-percentile -d 'Which percentile of the input image brightness histogram to consider as the t…'
complete -c mpv -l hdr-peak-decay-rate -d 'The decay rate used for the HDR peak detection algorithm (default: 20. 0)'
...
complete -c mpv -l hdr-contrast-recovery -l hdr-contrast-smoothness -d 'Enables the HDR contrast recovery algorithm, which is to designed to enhance …'
...
complete -c mpv -l icc-cache -d 'Store and load 3DLUTs created from the ICC profile on disk in the cache direc…'
complete -c mpv -l icc-cache-dir -d 'The directory where icc cache is stored'
...
complete -c mpv -l icc-force-contrast -d 'Override the target device\'s detected contrast ratio by a specific value'
complete -c mpv -l icc-use-luma -d 'Use ICC profile luminance value.  (Only for --vo=gpu-next)'
complete -c mpv -l lut -d 'Specifies a custom LUT (in Adobe '
complete -c mpv -l lut-type -d 'Controls the interpretation of color values fed to and from the LUT specified…'
complete -c mpv -l blend-subtitles -d 'Blend subtitles directly onto upscaled video frames, before interpolation and…'
complete -c mpv -l background -d 'If the frame has an alpha component, decide what kind of background, if any, …'
complete -c mpv -l background-color -d 'Color used to draw parts of the mpv window not covered by video'
complete -c mpv -l border-background -d 'Same as --background but only applies to the black bar/border area of the win…'
...
complete -c mpv -l gpu-shader-cache -d 'Store and load compiled GLSL shaders in the cache directory (Default: yes)'
complete -c mpv -l gpu-shader-cache-dir -d 'The directory where gpu shader cache is stored'
complete -c mpv -l libplacebo-opts -d 'Passes extra raw option to the libplacebo rendering backend (used by --vo=gpu…'
complete -c mpv -l display-tags -d 'Set the list of tags that should be displayed on the terminal and stats'
...
complete -c mpv -l video-sync-max-factor -d 'Maximum multiple for which to try to fit the video\'s FPS to the display\'s FPS…'
...
complete -c mpv -l backdrop-type -d '(Windows only) Controls the backdrop/border style.  NDENT 7. 0'
complete -c mpv -l window-affinity -d '(Windows only) Controls the window affinity behavior of mpv.  NDENT 7. 0'
complete -c mpv -l vo-mmcss-profile -d '(Windows only) Set the MMCSS profile for the video renderer thread (default: …'
complete -c mpv -l priority -d '(Windows only) Set process priority for mpv according to the predefined prior…'
complete -c mpv -l media-controls -d '(Windows only) Enable integration of media control interface SystemMediaTrans…'
...
complete -c mpv -l image-exts -d 'Image file extentions to try to match when using --cover-art-auto, --autocrea…'
complete -c mpv -l cover-art-whitelist -d 'Filenames to load as cover art, sorted by descending priority'
complete -c mpv -l video-exts -d 'Video file extentions to try to match when using --autocreate-playlist or --d…'
...
complete -c mpv -l metadata-codepage -d 'Codepage for various input metadata (default: auto)'
...
complete -c mpv -o toggle -d 'T}	T{ Append an item, or remove it if it already exists (no escapes) T}'
complete -c mpv -o help -d 'T}	T{ Pseudo operation that prints a help text to the terminal T}'
...
complete -c mpv -l watch-later-directory
complete -c mpv -l watch-later-options-remove -d 'The subtitle track selection will not be restored.  o 2'
complete -c mpv -l no-video-aspect-override
complete -c mpv -o '0.1'
complete -c mpv -s 2 -d 'the size, and so on'
complete -c mpv -o ao -d 'In mpv you could instead use:'
complete -c mpv -l sub-border-color
complete -c mpv -l sub-shadow-color
complete -c mpv -l sub-border-size
complete -c mpv -l sub-filter-regex-append -d 'new regular expression, without having to fight escaping problems'
complete -c mpv -l osd-bar-border-size
complete -c mpv -l osd-border-color
complete -c mpv -l osd-shadow-color
complete -c mpv -l osd-border-size
complete -c mpv -l screenshot-directory
...
complete -c mpv -l alsa-ignore-chmap -d 'Don\'t read or set the channel map of the ALSA device - only request the requi…'
...
complete -c mpv -l pulse-latency-hacks -d 'Enable hacks to workaround PulseAudio timing bugs (default: yes)'
complete -c mpv -l pulse-allow-suspended -d 'Allow mpv to use PulseAudio even if the sink is suspended (default: no)'
complete -c mpv -l pipewire-buffer -d 'Set the audio buffer size in milliseconds'
complete -c mpv -l pipewire-remote -d 'Specify the PipeWire remote daemon name to connect to via local UNIX sockets'
complete -c mpv -l pipewire-volume-mode -d 'Specify if the ao-volume property should apply to the channel volumes or the …'
...
complete -c mpv -l wasapi-exclusive-buffer -d 'Set buffer duration in exclusive mode (i. e. , with'
...
complete -c mpv -l vo-direct3d-exact-backbuffer -d 'Always resize the backbuffer to window size.  sdl SDL 2'
...
complete -c mpv -l vo-tct-buffering -d 'Specifies the size of data batches buffered before being sent to the terminal'
...
complete -c mpv -l vo-kitty-cols -l vo-kitty-rows -d 'Specify the terminal size in character cells, otherwise (0) read it from the …'
complete -c mpv -l vo-kitty-width -l vo-kitty-height -d 'Specify the available size in pixels, otherwise (0) read it from the terminal…'
complete -c mpv -l vo-kitty-left -l vo-kitty-top -d 'Specify the position in character cells where the image starts (1 is the firs…'
complete -c mpv -l vo-kitty-config-clear -d 'Whether or not to clear the terminal whenever the output is reconfigured (e'
complete -c mpv -l vo-kitty-alt-screen -d 'Whether or not to use the alternate screen buffer and return the terminal to …'
complete -c mpv -l vo-kitty-use-shm -d 'Use shared memory objects to transfer image data to the terminal'
...
complete -c mpv -l vo-sixel-config-clear -d 'Whether or not to clear the terminal whenever the output is reconfigured (e'
complete -c mpv -l vo-sixel-alt-screen -d 'Whether or not to use the alternate screen buffer and return the terminal to …'
complete -c mpv -l vo-sixel-exit-clear -d 'may be removed in the future'
complete -c mpv -l vo-sixel-buffered -d 'Buffers the full output sequence before writing it to the terminal'
...
complete -c mpv -l drm-drmprime-video-plane -d 'Select the DRM plane to use for video with the drmprime-overlay hwdec interop…'
...
complete -c mpv -l drm-vrr-enabled -d 'Toggle use of Variable Refresh Rate (VRR), aka Freesync or Adaptive Sync on c…'
complete -c mpv -l af-clr
...
complete -c mpv -l vf-toggle -d 'Add the given filter to the list if it was not present yet, or remove it from…'
complete -c mpv -l vf-clr -d 'Completely empties the filter list'
...
complete -c mpv -l ocopy-metadata -d 'Copy metadata from input files to output files when encoding (default: yes)'
