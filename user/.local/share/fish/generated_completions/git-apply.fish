...
complete -c git-apply -l allow-binary-replacement -l binary -d 'Historically we did not allow binary patch application without an explicit pe…'
...
complete -c git-apply -l allow-empty -d 'Don\'t return an error for patches containing no diff'
