...
complete -c docker-container-run -l annotation -d '	Add an annotation to the container (passed through to the OCI runtime)'
...
complete -c docker-container-run -l cgroupns -d '	Cgroup namespace to use (host|private) \'host\':    Run the container in the D…'
...
complete -c docker-container-run -l health-start-interval -d '	Time between running the check during the start period (ms|s|m|h) (default 0…'
...
complete -c docker-container-run -l rm -d '	Automatically remove the container and its associated anonymous volumes when…'
