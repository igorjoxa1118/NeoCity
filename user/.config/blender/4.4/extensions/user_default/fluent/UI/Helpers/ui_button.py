...
class FLUENT_Ui_Button:
    def __init__(self, name=None):
        ...
        self.margin = 12 * get_addon_preferences().interface_factor
        ...
        try:
            self.icon_size = [int(32 * get_addon_preferences().interface_factor), int(32 * get_addon_preferences().interface_factor)]
        except:
    ...
    def refresh_size(self):
        if self.text == '' and self.icon:
            ...
            self.dimensions = [32 * get_addon_preferences().interface_factor, 32 * get_addon_preferences().interface_factor]
        ...
        if self.show:
            ...
            if self.shape == 'RECTANGLE':
                if self.state in [1, 2, 3]:
                    draw_rounded_rectangle(x=self.position[0], y=self.position[1], width=self.dimensions[0], height=self.dimensions[1], color=self.outline_color(), align=self.align, radius=8, resolution=int(6* get_addon_preferences().interface_factor))
                    draw_rounded_rectangle(x=self.position[0], y=self.position[1], width=self.dimensions[0]-4, height=self.dimensions[1]-4, color=self.set_colors(), align=self.align, radius=6, resolution=int(6* get_addon_preferences().interface_factor))
                else:
                    draw_rounded_rectangle(x=self.position[0], y=self.position[1], width=self.dimensions[0], height=self.dimensions[1], color=self.set_colors(), align=self.align, radius=8, resolution=int(6* get_addon_preferences().interface_factor))
            elif self.shape == 'CIRCLE':
                if self.state in [1, 2, 3]:
                    draw_circle_full(cx=self.position[0], cy=self.position[1], r=self.dimensions[0] / 2, color=self.outline_color(), segments=int(64 * get_addon_preferences().interface_factor))
                    draw_circle_full(cx=self.position[0], cy=self.position[1], r=self.dimensions[0] / 2 - 2, color=self.set_colors(), segments=int(64 * get_addon_preferences().interface_factor))
                else:
                    draw_circle_full(cx=self.position[0], cy=self.position[1], r=self.dimensions[0] / 2, color=self.set_colors(), segments=int(64 * get_addon_preferences().interface_factor))
...
class FLUENT_Ui_Layout:
    def __init__(self, id, title='', subtitle=''):
        ...
        self.margin = 16 * get_addon_preferences().interface_factor
        self.rayon_du_pie = 150 * get_addon_preferences().interface_factor
