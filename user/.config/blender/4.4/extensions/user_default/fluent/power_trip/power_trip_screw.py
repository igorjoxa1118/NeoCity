...
Select an object before."""
    ...
    def adjustment(self):
        ...
        if 'SCALE' in what:
            if events['shift_work']:
                increment = 12000*get_addon_preferences().interface_factor
            elif events['ctrl_work']:
                increment = 120*get_addon_preferences().interface_factor
            else:
                increment = 1200*get_addon_preferences().interface_factor
        ...
        if 'OFFSET' in what:
            if events['shift_work']:
                increment = 8000*get_addon_preferences().interface_factor
            elif events['ctrl_work']:
                increment = 80*get_addon_preferences().interface_factor
            else:
                increment = 800*get_addon_preferences().interface_factor
    ...
    def modal(self, context, event):
        ...
        if self.status == 'WAIT_SELECTION' and event.type in ['RET', 'NUMPAD_ENTER'] and event.value == 'PRESS':
            ...
            self.screw_bool['fluent_secondary_type'] = 'screw_bool'
    ...
    def invoke(self, context, event):
        ...
        if self.operation == 'ADD':
            ...
            screw_bool_modifiers = find_screw_bool_modifier(self.copy)
            apply_modifiers(self.copy, ignore=screw_bool_modifiers)
            self.copy.modifiers.clear()
