...
class FLUENT_Drawing():
    ...
    def __init__(self):
        ...
        self.snap_distance = 16*get_addon_preferences().interface_factor
    ...
    def grid_init(self, obj_original, events):
        ...
        if obj_original and not self.align_to_view:
            ...
            precision = 2
            rounded_x = math.fabs(round(self.drawing_plane.rotation_euler.x, precision))
            rounded_y = math.fabs(round(self.drawing_plane.rotation_euler.y, precision))
            rounded_angles = [round(0, precision), round(math.pi, precision), round(math.pi/2, precision)]

            test_x = False
            test_y = False

            if rounded_x in rounded_angles:
                test_x = True
            if rounded_y in rounded_angles:
                test_y = True
        ...
        if self.display_grid:
            ...
            if self.display_dots:
                ...
                try:
                    ...
                    draw_all_circle_full(grid_dots, self.dot_size*get_addon_preferences().interface_factor, self.dot_color, 8)
                except:pass
                try:
            ...
            if self.display_dots and len(close_list):
                ...
                for c in close_list:
                    if c['dist'] < min:
                        min = c['dist']
                        snaped = c
                draw_circle_full(snaped['coord'].x, snaped['coord'].y, self.dot_size*3*get_addon_preferences().interface_factor, self.dot_color, segments = 8)
