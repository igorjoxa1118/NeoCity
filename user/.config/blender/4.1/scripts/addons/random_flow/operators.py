...
class OBJECT_OT_r_loop_extrude(Operator):
	...
	def execute(self, context):
		...
		for i in range(0, max(loop_count)):
			limit_smooth = self.lshading_num[i] > 2
	...
	def draw(self, context):
		...
		col.label(text="Shading: 1. None 2. Angle 3. Base 4. Top")
...
class OBJECT_OT_r_panels(Operator):
	...
	limit_smooth : EnumProperty(
		...
		items = (
			...
			('ANGLE', 'Angle','Sharpen based on angle threshold'),
	...
	def execute(self, context):
		...
		if self.type == 'INSET':
			...
			limit_smooth = not self.limit_smooth in ['NONE', 'ANGLE']
			...
			if not self.only_msharp:
				...
				for i, faces in enumerate(cells):
					if limit_smooth:
						cells_edges = set(sum([list(f.edges) for f in faces], []))
						for e in cells_edges:
							e.smooth = True
			...
			if merge_pnl:
				...
				for e in bm_src.edges:
					co = (e.verts[0].co + e.verts[1].co) / 2
					if co in bounds_co:
						e.smooth = False if self.limit_smooth == 'BASE' else True
						e.seam = True
				...
				if not self.base_smooth:
					if self.area_smooth:
						mesh_smooth(bm_src, [v for v in bm_src.verts if v.co in ref_vrt], self.cuts_smooth, self.smooth_iter)
					else:
						mesh_smooth(bm_src, bm_src.verts, self.cuts_smooth, self.smooth_iter)
			...
			if link_obj:
				...
				if not limit_smooth: set_sharp(obj, new_obj)
			...
			bvl = bmesh.ops.bevel(
				bm,
				geom            = undupe(split_edg + sharp_edg),
		...
		if self.origin == 'FACES':
			...
			for i, v in enumerate(origin_verts):
				...
				if random() < self.birth_perc/100 \
					and not v in vv:
					self.add_screw(context, obj, bm, v[0], v[1], i)
					vv.append(v)
