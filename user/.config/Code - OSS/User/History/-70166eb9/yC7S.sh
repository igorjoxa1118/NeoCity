...
sudo pacman -S dialog cmus xfce4-terminal thunar polybar rofi dunst nitrogen fzf mcfly neofetch
...
cd $PWD/sources/oh-my-zsh-git
   ...
   sudo pacman -S zsh zsh-syntax-highlighting zsh-history-substring-search zsh-syntax-highlighting starship
   makepkg -s
   fi
   cd ..
  else
...
do if [[ -d $dir ]]; then # Делать, если существуют или файлы
  mv $dir $BAK_DIR/home
  fi
done
