...
function info_playlist() { # При нажатии на кнопку "Аудио", плеер запускает поиск плейлиста URL из youtube и пускает вывод в StdOut/StdErr
 mpv --fs -ao=pulse --no-video --shuffle --script-opts-append=osc-visibility=always --term-playing-msg='Title: ${media-title}' "$input" | yad -tail --> log_play_info.log --> yad_main_display
}
...
function info_track() { # При нажатии на кнопку "Аудио", плеер запускает поиск плейлиста из youtube и пускает вывод в StdOut/StdErr
 mpv --fs ytdl://ytsearch:"$input" --no-video -ao=pulse --script-opts-append=osc-visibility=always --term-playing-msg='Title: ${media-title}' | yad -tail --> log_play_info.log --> yad_main_display
}
