...
if [ "$(id -u)" = 0 ]; then
    echo "${LIGHTBLUE}This script MUST NOT be run as root user."
    exit 1
fi
...
if [ ! -d "$repo_dir" ]; then
   echo -e "${LIGHTBLUE}Cloning dotfiles in %s\n" "$repo_url"
...
logo "Backup files"
echo -e "${LIGHTBLUE}Backup files will be stored in %s%s%s/.Backup_files%s \n\n" "${BLD}" "${CRE}" "$HOME" "${CNC}"
...
echo -e "${GRE}Power supply install done!"
sleep 2
clear
...
echo -e "${GRE}Connection interfaces install done!"
sleep 2
clear
...
for ff_themes in $repo_dir/firefox/*; do
  ...
  if [ $? -eq 0 ]; then
	echo -e "${LIGHTBLUE}Firefox theme install done!%s\n"
	sleep 1
  else
	echo -e "${LIGHTBLUE}Failed to been copied, you must copy it manually"
	sleep 1
  fi
done
fi
...
if [ ! -z "$grep_ff" ]; then
   ...
   echo -e "${LIGHTBLUE}Please start FF befor run this script"
   exit 1
fi
...
echo -e "${LIGHTBLUE}Enabling and starting the user-level mpd service"
...
shell_change() {
  logo "Changing default shell to zsh"
	if [[ $SHELL != "/usr/bin/zsh" ]]; then
		echo -e "${LIGHTBLUE}Changing your shell to zsh. Your root password is needed."
		# Переключиться на zsh
		chsh -s /usr/bin/zsh
		echo -e "${LIGHTBLUE}Shell changed to zsh. Please reboot."
	else
		echo -e "${LIGHTBLUE}Your shell is already zsh! Installation finished, now reboot"
	fi
}
