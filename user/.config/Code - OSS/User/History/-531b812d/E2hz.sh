...
if [ ! -d "$repo_dir" ]; then
   echo -e "${BLUE}Cloning dotfiles in %s\n" "$repo_url"
...
sed -i "s/enp59s0/${en_int}/g" "$HOME"/.config/i3/polybar/Tokio_night/modules
  ...
  read -p "${PURPLE}What is you Wired connection interface?(Example: eth0, enp59s0): " et_int_custom
...
read -p "${PURPLE}What is you Wireless connection interface?(Example: wlan0, wlp0s20f3): " wl_int_custom
sed -i "s/wlp0s20f3/${wl_int_custom}/g" "$HOME"/.config/i3/polybar/Tokio_night/modules
fi
}
echo -e "${PURPLE}Connection interfaces install done!"
sleep 2
clear
...
nvidia_detect()
{
  ...
  blacklight=$(ls -1 /sys/class/backlight/)
    ...
    if [ $(lspci -k | grep -A 2 -E "(VGA|3D)" | grep -i nvidia | wc -l) -gt 0 ]; then
        echo -e "${ORANGE}Nvidia driver found!"
    else
        ...
        echo -e "${CYAN}Nvidia driver no found!"
    fi
}
nvidia_detect
sleep 2
