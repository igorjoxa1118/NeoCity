...
if [ ! -d "$repo_dir" ]; then
   echo -e "Cloning dotfiles in %s\n" "$repo_url"
   git clone --depth=1 "$repo_url" "$repo_dir"
   else
   echo -e "Dotfile folder exist"
   sleep 1
   cd "$repo_dir"
   git pull
   echo -e "Update done"
...
logo "Backup files"
echo -e "Backup files will be stored in %s%s%s/.Backup_files%s \n\n" "${BLD}" "${CRE}" "$HOME" "${CNC}"
...
for ff_themes in $repo_dir/firefox/*; do
  ...
  if [ $? -eq 0 ]; then
	echo -e "Firefox theme install done!%s\n"
	sleep 1
  else
	echo -e "Failed to been copied, you must copy it manually"
	sleep 1
  fi
done
fi
...
if [ ! -z "$grep_ff" ]; then
   ...
   echo -e "Please start FF befor run this script"
   exit 1
fi
