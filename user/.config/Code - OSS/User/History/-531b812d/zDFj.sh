...
if [ "$(id -u)" = 0 ]; then
    echo -e "${CYAN}This script MUST NOT be run as root user."
    exit 1
fi
...
# Проверка того, что архив user-dirs.dirs не существует в ~/.config
	if [ ! -e "$HOME/.config/user-dirs.dirs" ]; then
		...
		echo "${CYAN}Creating xdg-user-dirs"
	fi
sleep 2 
clear
...
if [ ! -d "$repo_dir" ]; then
   ...
   echo "${CYAN}Dotfile folder exist"
   sleep 1
   cd "$repo_dir"
   git pull
   echo "${CYAN}Update done"
