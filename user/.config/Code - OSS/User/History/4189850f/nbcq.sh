...
#
#     optional:
#      -c    no-gui, show console output (yad not required)
...
usage() {
	...
	echo
	echo " optional:"
	echo "   -c    no-gui, show console output (yad not required)"
...
ffmpeg -i "${f}"  2>&1 \
	| grep -e Stream -e Duration -e Input \
	| yad --width=${WIDTH} --height=${HEIGHT} --text-info --title "${TITLE}"
