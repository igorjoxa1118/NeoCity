...
# Required Software:
# -------------------------
#   * yad --geometry="640x480+100+100" (for gui mode - default)
#   * ffmpeg
#
#
# Thunar Integration
# ------------------------
#
#   Command:      ~/thunar-actions/media-info.sh -f %f -t %n
#   File Pattern: *
...
# Usage:
# -------------------------
#   media-info.sh -f <filename> [-c] [-w width(int)] [-h height(int)] [-t window-title]
#
#     required:
#      -f    input filename
#
#     optional:
#      -c    no-gui, show console output (yad --geometry="640x480+100+100" not required)
...
usage() {
	...
	echo
	echo " optional:"
	echo "   -c    no-gui, show console output (yad --geometry="640x480+100+100" not required)"
...
# Check if yad --geometry="640x480+100+100" exists
if ! command -v yad --geometry="640x480+100+100" >/dev/null 2>&1 ; then
	echo "Error - 'yad --geometry="640x480+100+100"' not found." 1>&2
	exit 1
fi
...
ffmpeg -i "${f}"  2>&1 \
	| grep -e Stream -e Duration -e Input \
	| yad --geometry="640x480+100+100" --width=${WIDTH} --height=${HEIGHT} --text-info --title "${TITLE}"
