...
read -p "Какой оконный менеджер хотите забэкапить? i3wm...bspwm : " dwm
...
if [ -d ""$backup_drive"/.config" ]; then
    echo -e "${Purple}Удаление файлов на USB началось."
    rm -rf "$backup_drive"/.*
    clear

    echo -e "${Cyan}Копирование файлов в USB началось. Необходимо скопировать примерно "$size""
    ...
    echo -e "${Green}Копирование файлов в USB началось. Необходимо скопировать примерно "$size""
    rsync -aAEHSXxr --stats -h --info=progress2 --info=name0 /home/"$username"/.[^.]* "$backup_drive"
    sleep 2
    clear

    echo -e "${Red}Синхронизация!"
    sync
    clear
