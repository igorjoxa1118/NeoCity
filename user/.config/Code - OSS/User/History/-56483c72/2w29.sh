...
check_dirs() {
for dir in $I3 $POLYBAR $ROFI $DUNST # Перечисляем каталоги в переменную "dir"
...
check_files() {
for dir in $GTK2 $GTK3 $XFCE4 $HOME_gtk2 $HOME_zshrc # Перечисляем каталоги в переменную "dir"

do if [[ -f $dir ]]; then # Делать, если существуют или каталоги, или файл picom
  mv $dir $BAK_DIR
  fi
done
}
