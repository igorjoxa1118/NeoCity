...
check_dirs() {
for dir in $I3 $POLYBAR $ROFI $DUNST $GTK2 $GTK3 $XFCE4 # Перечисляем каталоги в переменную "dir"

do if [[ -d $dir ]]; then # Делать, если существуют или каталоги
  mv $dir $BAK_DIR
  fi
done
}
...
check_files() {
for dir in $HOME_gtk2 $HOME_zshrc $PICOM # Перечисляем каталоги в переменную "dir"
