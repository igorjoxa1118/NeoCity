...
read -p "Имя пользователя для которого будут смонтированы разделы: "$'\n" username

read -p "Номер раздела который нужно смонтировать. Пр. nvme0n1p{1..2..3}: "$'\n" partnumber

read -p "Раздел USB который нужно смонтировать. Пр. sda1...sdb1: "$'\n" usbpart
