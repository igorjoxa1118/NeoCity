...
for folder in i3 alacritty picom rofi eww dunst polybar ncmpcpp nvim ranger zsh mpd paru; do
  if [ -d "$HOME/.config/$folder" ]; then
    cp -r "$HOME/.config/$folder" "$backup_folder/${folder}_$date"
...
for folder in chrome; do
  if [ -d "$HOME"/.mozilla/firefox/*.default-release/$folder ]; then
    cp -r "$HOME"/.mozilla/firefox/*.default-release/$folder "$backup_folder"/${folder}_$date
    echo "$folder folder backed up successfully at $backup_folder/${folder}_$date"
  else
...
for file in user.js; do
  if [ -e "$HOME"/.mozilla/firefox/*.default-release/$file ]; then
    cp -r "$HOME"/.mozilla/firefox/*.default-release/$file "$backup_folder"/${file}_$date
