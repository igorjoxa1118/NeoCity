...
if [ ! -f "$HOME"/Загрузки/repository/sources_test/samba_config/test_for_touch/smb.conf ]; then
    ...
    touch "$HOME"/Загрузки/repository/sources_test/samba_config/test_for_touch/smb2.conf
    cat smb.conf > "$HOME"/Загрузки/repository/sources_test/samba_config/test_for_touch/smb2.conf
else
    echo "File exist!"
fi