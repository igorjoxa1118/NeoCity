...
logo "Welcome!"
printf '%s%sPlease, start and closed your Firefox if you have it. If not, firefox theme not be installed first time.\nThis script will check if you have the necessary dependencies, and if not, it will install them.%s\n\n' "${BLD}" "${CRE}" "${CNC}"
...
if [ ! -d "$backup_folder" ]; then
  ...
  mkdir "$backup_folder/firefox"
fi

for folder in blender alacritty picom rofi eww gtk-2.0 gtk-3.0 gtk-4.0 i3 kitty Kvantum lsd micro musikcube neofetch ranger stalonetray xfce4 dunst polybar ncmpcpp nvim zsh mpd yay; do
  if [ -d "$HOME/.config/$folder" ]; then
    cp -r "$HOME/.config/$folder" "$backup_folder/${folder}"
    echo "$folder folder backed up successfully at $backup_folder"
  else
    echo "The folder $folder does not exist in $HOME/.config/"
  fi
done

for folder_FF in chrome; do
  if [ -d "$HOME"/.mozilla/firefox/*.default-release/$folder_FF ]; then
    cp -r "$HOME"/.mozilla/firefox/*.default-release/$folder_FF "$backup_folder/firefox"
    echo "$folder_FF folder backed up successfully at $backup_folder/firefox/"
  else
    echo "The folder $folder does not exist in $HOME/.mozilla/firefox/"
  fi
done

for file_FF in user.js; do
  if [ -e "$HOME"/.mozilla/firefox/*.default-release/$file_FF ]; then
    cp -r "$HOME"/.mozilla/firefox/*.default-release/$file_FF "$backup_folder/firefox"
    echo "$file file backed up successfully at $backup_folder/firefox/"
  else
    echo "The file $file_FF does not exist in $HOME/.mozilla/firefox/"
  fi
done

[ -f ~/.zshrc ] && cp ~/.zshrc ~/.Backup_files/.zshrc
[ -f ~/.gtkrc-2.0 ] && cp ~/.zshrc ~/.Backup_files/.gtkrc-2.0
[ -f ~/.config/picom.conf ] && cp ~/.zshrc ~/.Backup_files/.gtkrc-2.0
