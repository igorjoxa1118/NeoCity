...
Первый раз это нужно для того, что бы Firefox создал свои файлы и каталоги настроек в ~/.mozilla/, $${\color{lightgreen}поэтому \space важно \space запустить \space Firefox \space и \space закрыть.}$$ Затем перезапустить скрипт
