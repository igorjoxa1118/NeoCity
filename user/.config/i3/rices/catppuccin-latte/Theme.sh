...
* {
    ...
    /* Gradients */
    gradient-1:                  linear-gradient(45, #e64553, #a6da95);
    gradient-2:                  linear-gradient(0, #eba0ac, #7A72EC);
    gradient-3:                  linear-gradient(70, #f9e2af, #eba0ac);
    gradient-4:                  linear-gradient(135, #74c7ec, #e64553);
    gradient-5:                  linear-gradient(to left, #bdc3c7, #2c3e50);
    gradient-6:                  linear-gradient(to right, #4c4f69, #4c4f69, #4c4f69);
    ...
    gradient-11:                 linear-gradient(0, #e64553, #79CBCA, #E684AE);
    ...
    gradient-18:                 linear-gradient(0, #e64553, #74c7ec, #74c7ec, #e64553);
...
Net/IconThemeName "Magna-Dark-Icons"
Gtk/CursorThemeName "catppuccin-mocha-maroon-cursors"
EOF
    ...
    for config in "$gtk2_config" "$gtk3_config" "$gtk4_config"; do
        if [ -f "$config" ]; then
            sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=Magna-Dark-Icons/g" "$config"
    ...
    if [[ -f "$gtk_settings_file" ]]; then
        ...
        sed -i "s/^gtk-icon-theme-name\s*=\s*.*/gtk-icon-theme-name = Magna-Dark-Icons/" "$gtk_settings_file"
        sed -i "s/^gtk-cursor-theme-name\s*=\s*.*/gtk-cursor-theme-name = catppuccin-mocha-maroon-cursors/" "$gtk_settings_file"
...
# Функция замены цветов cava на акцентные catppuccin latte
cava_colors() {
sed -i '/colors=(/,/)/c\
colors=(\
    "#1e66f5" "#d20f39" "#fe640b" "#40a02b" "#179299"\
    "#04a5e5" "#8839ef" "#dd7878" "#1e66f5" "#d20f39"\
    "#fe640b" "#40a02b" "#179299" "#04a5e5" "#8839ef"\
...
log "Скрипт для Latte успешно завершен!"