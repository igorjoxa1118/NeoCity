...
* {
    ...
    /* Gradients */
    gradient-1:                  linear-gradient(45, #ef9f76, #a6d189);
    gradient-2:                  linear-gradient(0, #eba0ac, #7A72EC);
    gradient-3:                  linear-gradient(70, #f9e2af, #eba0ac);
    gradient-4:                  linear-gradient(135, #74c7ec, #ef9f76);
    gradient-5:                  linear-gradient(to left, #bdc3c7, #2c3e50);
    gradient-6:                  linear-gradient(to right, #303446, #303446, #303446);
    ...
    gradient-10:                 linear-gradient(0, #283c86, #a6d189);
    gradient-11:                 linear-gradient(0, #ef9f76, #79CBCA, #E684AE);
    ...
    gradient-15:                 linear-gradient(0, #a6d189, #a6d189);
    gradient-16:                 linear-gradient(0, #232526, #414345);
    gradient-17:                 linear-gradient(0, #833ab4, #eba0ac, #f9e2af);
    gradient-18:                 linear-gradient(0, #ef9f76, #74c7ec, #74c7ec, #ef9f76);
    ...
    background-selected:         #ef9f76;
    foreground-normal:           #cdd6f4;
    foreground-selected:         #303446;
}
EOF
}
rofi_powermenu
...
set_icons() {
    ...
    sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name="\"Catppuccin-Frappe"\"/g" "$HOME"/.gtkrc-2.0
    sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=Catppuccin-Frappe/g" "$HOME"/.config/gtk-3.0/settings.ini
    sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=Catppuccin-Frappe/g" "$HOME"/.config/gtk-4.0/settings.ini
}
set_icons
...
set_network_manager() {
	sed -i 's|@import "colors/catppuccin-[^"]*"|@import "colors/catppuccin-frappe.rasi"|g' "$HOME/.config/i3/src/rofi-themes/NetManagerDM.rasi"
}
set_network_manager
