...
set_icons() {
    ...
    for config in "$gtk2_config" "$gtk3_config" "$gtk4_config"; do
        if [ -f "$config" ]; then
            sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=Catppuccin-Frappe/g" "$config"
...
set_cursor() {
    ...
    for config in "$gtk2_config" "$gtk3_config" "$gtk4_config"; do
        if [ -f "$config" ]; then
            sed -i "s/gtk-cursor-theme-name=.*/gtk-cursor-theme-name=catppuccin-mocha-teal-cursors/g" "$config"
