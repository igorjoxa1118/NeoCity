...
EOF
    ...
    for config in "$gtk2_config" "$gtk3_config" "$gtk4_config"; do
        if [ -f "$config" ]; then
            ...
            sed -i "s/Inherits=.*/Inherits=catppuccin-mocha-peach-cursors/g" "$default_cursor_file"
    ...
    if [[ -f "$gtk_settings_file" ]]; then
        ...
        sed -i "s/^gtk-icon-theme-name\s*=\s*.*/gtk-icon-theme-name = Catppuccin-Macchiato/" "$gtk_settings_file"
